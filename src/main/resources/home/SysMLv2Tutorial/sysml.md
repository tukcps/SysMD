...
title: SysML v2 Textual Representation
...
From KerML, it uses in particular its semantic library that defines different kind 
of Classifiers (Classes, Datatypes) and Features. 

All SysMLv2 artefacts are, in the end, specialization of these Classifiers and Features. 

 
## Language design


In SysMLv2 the difference between a _definition_ and a _usage_ is concept that is found in each
construct: 

- A _definition_ uses the keyword **def** after a keyword that specifies the kind of artefact that is defined
  and introduces a kind of Class. 
- A _usage_ just uses the keyword that specifies the kind of artefact without _def_. 
...
but as well a user-defined data type. 
...
- A car that is a vehicle with body, four wheels an one or two engines 
  (e.g. electrical and compuustion).  
...
A requirements begins with the keyword ```requirement```.
...
A requirement definition allows users to create a class of requirement with a specific infrastructure that is inherited to each usage of 
...
```
## Requirement Usage 
```SysMD::tutorial::sysml::requirementsExample
    part p: Box; 
