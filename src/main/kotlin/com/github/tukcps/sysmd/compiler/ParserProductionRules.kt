...
abstract class ParserProductionRules(
    ...
    inner class alternatives(cases: alternatives.() -> Unit) {
        ...
        init {
            ...
            when {
                ...
                else -> error("expected $expected, but parsed $t1 ") // no others registered & no match ...
            }
        }
     ...
     * (production)+
     * Guided by token required at start resp. at the end (a lambda with boolean results).
     * @param stop tokens that stop production
     * @param production a production implementation as lambda
     */
    inline fun oneOrMoreUntil(stop: Set<Token.Kind>, production: () -> Unit ) {
        do {
            production()
        } while ( token.kind !in stop)
    }
