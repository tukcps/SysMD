...
object SyntaxHighlighter {
    ...
    private fun annotateSyntaxHighlighting(text: String, colorScheme: ColorScheme): AnnotatedString {
        ...
        val scanner = Scanner(input = text, skip = emptySet())

        do {
            tokens.add(
                AnnotatedString(
                when(scanner.token.kind) {
                    REGULAR_COMMENT -> "/*${scanner.token.string}*/"
                    STRING_LIT -> "\""+scanner.token.string+"\""
                    NAME_LIT -> text.substring(scanner.token.indices)
                    ERROR -> return AnnotatedString(text) //Simple fix to cope with errors in text
                    else -> scanner.token.string
                },
                when (scanner.token.kind) {
                    in Token.keywords.values -> SpanStyle(colorScheme.primary)
                    ERROR -> SpanStyle(colorScheme.error)
                    COMMENT,REGULAR_COMMENT,NOTE -> SpanStyle(Color.Gray)
                    STRING_LIT -> SpanStyle(color = colorScheme.secondary, fontStyle = FontStyle.Italic)
                    FLOAT_LIT -> SpanStyle(color = colorScheme.secondary, fontStyle = FontStyle.Italic)
                    INTEGER_LIT -> SpanStyle(color = colorScheme.secondary, fontStyle = FontStyle.Italic)
           ...
           (offset > -1 && offset < 1) -> {
                ...
                when {
                    ...
                    /** Standard Case: Cursors is somewhere inside the text*/
                        ...
                        val block2 = annotateSyntaxHighlighting(newTFVShadowed.text.substring(firstIndex, min(lastIndex, newTFVShadowed.text.length)),colorScheme) //Line that is edited, gets new Syntax Highlighting
