...
fun Session.report(element: Element?, message: String, cause: Throwable? = null) {
    ...
    status.exceptions.add(
        if (cause is SysMDException) {
            if (cause.textualRepresentation == null)
                cause.textualRepresentation = textualRepresentation?.ref as TextualRepresentation?
            cause.element = element
            // val eq = cause.equals(status.exceptions.firstOrNull())
            cause
        } else SysMDError(
