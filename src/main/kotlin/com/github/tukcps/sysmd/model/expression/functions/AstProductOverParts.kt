...
/**
 * The productOverParts function with parameter propertyAST.
 ...
 */
internal class AstProductOverParts(
    model: Session,
    ...
    override fun clone(): AstProductOverParts {
        return AstProductOverParts(model, namespace, listOf(propertyAst.first().clone()), transitive)
    }
}
...
fun Session.initProductParts(
    ...
    for (elementIterator in element.getOwnedElementsOfType<Feature>().filterNot { it is Variable || it.variable is Variable }) {
        ...
        for (leaf in newAstNode.getLeaves().filter { it.qualifiedName != null }) {
            ...
            val variable = (elementIterator as Namespace).resolveVar(leaf.qualifiedName as String)
            if (variable != null) {
