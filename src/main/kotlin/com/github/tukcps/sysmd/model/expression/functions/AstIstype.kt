...
class AstIstype(
    ...
    override fun evalUp() {
        ...
        if (subtype is Type && supertype is Type) {
            upQuantity = Quantity(
                if (supertype == subtype) model.builder.True else model.builder.False
            )
        } else
            model.report(owningNamespace, "Evaluation of istype not possible as parameters are not types.")
    }
