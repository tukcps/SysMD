...
class QuantityTestsWithFunctions {
    ...
    fun sinCosTestNegative() = testSession("Ranges")  {
        ...
        solver.propagate()
        assertNoIssues()
        //assertEquals(sin(-0.5),solver.getVariable("b")!!.vectorQuantity.value.asAadd().min, 0.0001)
        //assertEquals(sin(-1.0),solver.getVariable("b")!!.vectorQuantity.value.asAadd().max, 0.0001)
        assertEquals(cos(-1.0),solver.getVariable("c")!!.vectorQuantity.value.asAadd().min, 0.0001)
        assertEquals(cos(-0.5),solver.getVariable("c")!!.vectorQuantity.value.asAadd().max, 0.0001)
    }
