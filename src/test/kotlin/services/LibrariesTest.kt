 ...
 * of these elements to the instances in all kinds of KerML models.
 */
@Isolated
class LibrariesTest {
    ...
    @Test
    fun librariesRepositoryTest() {
        // LibraryRepository.loadAllLibrariesFromRessources(status)
    }
    ...
    fun baseLibraryTest() = testSession {
        assertNotNull(LibraryRepository.getLibrary("Base", status))
    ...
    fun scalarValuesTest() = testSession {
        LibraryRepository.getArrangement("ScalarValues", listOf("Base", "ScalarValues"))
        assertNotNull(LibraryRepository.getArrangementFromCache("ScalarValues"))
    ...
    @Test @ResourceLock(value = SYSTEM_PROPERTIES, mode = READ_WRITE)
    fun kerMLLibraryTest() = testSession("") {
        loadLibraryArrangement("KerML")
        ...
        // assertEquals(UUIDPolicies.uuid5("KerML::Kernel::DataType"), dataType.elementId)
    ...
    /**
     * Maybe rather issue of test itself? Or checkConsistency?
     * Drop or re-write.
     */
    @Test @ResourceLock(value = SYSTEM_PROPERTIES, mode = READ_WRITE)
    fun objectsTest() = testSession() {
        loadLibraryArrangement("Objects")
        initialize(Runlevel.MODEL)
        assertNotNull(global.resolve("Objects::Object"))
        assertNoIssues()
        assertEquals(get(), repo.elements())
        // FIXME: Somewhere a link is deleted/cloned improperly, causing the FeatureTyping of its source/target features to dangle and reference invalid UUIDs
     ...
     */
    @Test @ResourceLock(value = SYSTEM_PROPERTIES, mode = READ_WRITE)
    fun allowedRecursionTest() = testSession("ScalarValues") {
        loadKerML("""
            standard library package tst {
    ...
    @Test @ResourceLock(value = SYSTEM_PROPERTIES, mode = READ_WRITE)
    fun allowedRecursionTest2() = testSession("ScalarValues") {
        loadKerML("""
                standard library package tst {
    ...
    @Test @ResourceLock(value = SYSTEM_PROPERTIES, mode = READ_WRITE)
    fun loadRangesLibraryTest() = testSession("ScalarValues") {
        loadKerML("""
                library package Ranges {
                   
                    // Abstract concept of a Range from which a Real is chosen
                    // Must only be used in combination with ScalarValues::Real. 
                    abstract datatype InRange :> ScalarValues::Real {
                        feature min: ScalarValues::Real;
                        feature max: ScalarValues::Real;
                        // inv selfIsInRange { (min <= self) and (self <= max) }
                    }
                   
                    abstract datatype OneInRange :> InRange; 
                    abstract datatype AllInRange :> InRange; 
                    
                    comment /* 
                    abstract datatype QuantityRange: InRange {
                        feature unit: Unit; 
                    }
                    
                    abstract datatype OneQuantityOf: QuantityRange; 
                    abstract datatype AllQuantityOf: QuantityRange; 
                    */ 
                }
            """)
        assertNoIssues()
                ...
                standard library package KerML {
                    ...
                    package Core {
                        ...
                        datatype FeatureDirectionKind {
                            // member feature not implemented. 
    ...
    @Test
    fun loadLibraryRanges() = testSession("ScalarValues", runlevel = Runlevel.NONE) {
        val inputStream = javaClass.getResourceAsStream("/libraries/Ranges.kerml")
        val inputString = inputStream?.bufferedReader().use { input -> input?.readText() }
        assertNotNull(inputString)
        loadKerML(inputString, Runlevel.MODEL)
        assertNoIssues()
        val ranges = global.resolve("Ranges")?.member<Package>()
        assertNotNull(ranges)
        val range = global.resolve("Ranges::InRange")?.member<DataType>()
        assertNotNull(range)
    }

    @Test
    fun loadLibraryLinks() = testSession("ScalarValues", runlevel = Runlevel.NONE) {
        val inputStream = javaClass.getResourceAsStream("/libraries/Links.kerml")
        val inputString = inputStream?.bufferedReader().use { input -> input?.readText() }
        assertNotNull(inputString)
        loadKerML(inputString, Runlevel.MODEL)
        assertNoIssues()
        val links = global.resolve("Links")?.member<Package>()
        assertNotNull(links)
    ...
    @Test
    fun loadLibraryOccurrences() = testSession("Links", runlevel = Runlevel.NONE) {
        val inputStream = javaClass.getResourceAsStream("/libraries/Occurrences.kerml")
        val inputString = inputStream?.bufferedReader().use { input -> input?.readText() }
        assertNotNull(inputString)
        loadKerML(inputString, Runlevel.MODEL)
        assertNoIssues()
        val occurrences = global.resolve("Occurrences")?.member<Package>()
        assertNotNull(occurrences)
        val happens = global.resolve("Occurrences::HappensBefore")?.member<Association>()
        assertNotNull(happens)
    }

    @Test
    fun loadLibraryISQ() = testSession("ScalarValues", runlevel = Runlevel.NONE) {

        // Ranges
        val inputStream1 = javaClass.getResourceAsStream("/libraries/Ranges.kerml")
        val inputString1 = inputStream1?.bufferedReader().use { input -> input?.readText() }
        assertNotNull(inputString1)
        loadKerML(inputString1, Runlevel.NONE)
        assertNoIssues()

        // Quantities
        val inputStream2 = javaClass.getResourceAsStream("/libraries/Quantities.kerml")
        val inputString2 = inputStream2?.bufferedReader().use { input -> input?.readText() }
        assertNotNull(inputString2)
        loadKerML(inputString2, Runlevel.NONE)
        assertNoIssues()

        val inputStream = javaClass.getResourceAsStream("/libraries/ISQ.kerml")
        val inputString = inputStream?.bufferedReader().use { input -> input?.readText() }
        assertNotNull(inputString)
        loadKerML(inputString, Runlevel.MODEL)
        assertNoIssues()

        val occurrences = global.resolve("ISQ")?.member<Package>()
        assertNotNull(occurrences)
        val happens = global.resolve("ISQ::LengthValue")?.member<DataType>()
        assertNotNull(happens)
    }
