...
class HasATests {
     ...
     */
    @Test
    fun sysMdCreatesValueFeature() = testSession("ScalarValues") {
     ...
     */
    @Test
    fun testHasACreation() = testSession("ISQ") {
        loadKerML(
            input = """
     ...
     */
    @Test
    fun hasARangeTest() = testSession("Occurrences", runlevel = Runlevel.ALL) {
        loadSysMD(
            """
        ...
        assertEquals(
            1,
            global.resolve("hasARange::Auto")?.member<Namespace>()?.getOwned<Feature>("motoren")?.multiplicityRange?.min
        )
        assertEquals(
            2,
            global.resolve("hasARange::Auto")?.member<Namespace>()?.getOwned<Feature>("motoren")?.multiplicityRange?.max
        )
        assertEquals(1, (solver.getVariable("hasARange::Auto::motoren::cardinality"))!!.intSpecs[0].min)
        assertEquals(2, (solver.getVariable("hasARange::Auto::motoren::cardinality"))!!.intSpecs[0].max)
        assertEquals(1.0, (solver.getVariable("hasARange::Auto::motoren::cardinality"))!!.min(), 0.0001)
        assertEquals(2.0, (solver.getVariable("hasARange::Auto::motoren::cardinality"))!!.max(), 0.0001)
    }
     ...
     */
    @Test
    fun hasATest1()  = testSession("ScalarValues") {
        loadKerML(
            """
     ...
     */
    @Test
    fun multiplicityTest() = testSession("ScalarValues") {
