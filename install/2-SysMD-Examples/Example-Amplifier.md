...
package OpAmpExample {
    part def OpAmp {
        attribute slewRate: Quantities::ScalarQuantityValue [1] {:>> range = 0..* [V/ms];}
    } 
    requirement def SlewRateRequirement {
        subject dut: OpAmp;
        attribute minSlewRate: Quantities::ScalarQuantityValue [1] {:>> range = 0..* [V/ms];}
...
Assume we are developing an amplifier. 
Let the system specification define three stages in a series.
...
package AmplifierExample {
    ...
    part def Amplifier {
        attribute gain: DimensionOneValue = [0.0 .. 100.0] dB {:>> range = 0..* [dB];}        
    }
    // Concrete model --> SystemC instances of library classes
    part myAmplifier {
        part lna:    Amplifier {
            attribute gain: DimensionOneValue = oneOf(15.0 .. 20.0 [dB]) {:>> range = 0..* [dB];}
        }
        part stage2: Amplifier {
            attribute gain: DimensionOneValue = oneOf(5.0 .. 20.0 [dB]) {:>> range = 0..* [dB];}
        }
        part driver: Amplifier {
            attribute gain: DimensionOneValue = oneOf(5.0 .. 20.0 [dB]) {:>> range = 0..* [dB];}  
        }
        attribute gain: DimensionOneValue = productOverParts(gain) { :>> range = 20..30 [dB];} 
    }
    ...
    part myAmplifier {
        part lna:    Amplifier {
            attribute gain: DimensionOneValue = characterizedResult(
                oneOf(15.0 .. 20.0 [dB]), "Amplifier/importResultsLna") {:>> range = 0..* [dB];}   
        }
        part stage2: Amplifier {
            attribute gain: DimensionOneValue = characterizedResult(
                oneOf(5.0 .. 20.0 [dB]), "Amplifier/importResultsStage2") {:>> range = 0..* [dB];}    
        }
        part driver: Amplifier {
            attribute gain: DimensionOneValue = characterizedResult(
                oneOf(0.0 .. 3.0 [dB]), "Amplifier/importResultsDriver") {:>> range = 0..* [dB];}     
        }
        attribute gain: DimensionOneValue = productOverParts(gain)  {:>> range = 20..30 [dB];}    
    }
}*/ 
